...
def roi_pooling_conv(img, rois, pooling_size, num_rois):
    ...
    :param num_rois: 
    :return: feature map与rois对应区域经过resize之后的特征映射结果
    """
    ...
    for num in range(num_rois):
        ...
        w = K.cast(w, 'int32')
        h = K.cast(h, 'int32')
        # RoI区域映射feature map的resize，（w * h）->（pooling_size, pooling_size）
        ##不在（w * h）上做（pooling_size, pooling_size）的区域划分，然后在每个区域上做max_pooling？
        rs = tf.image.resize_images(img[:, y:y + h, x:x + w, :], (pooling_size, pooling_size))
        outputs.append(rs)
    final_output = K.concatenate(outputs, axis=0)
    final_output = K.reshape(final_output, (1, num_rois, pooling_size, pooling_size, channels))
